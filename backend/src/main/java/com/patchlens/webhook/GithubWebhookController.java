...
public class GithubWebhookController {
    ...
    public ResponseEntity<Map<String, Object>> receive(
        ...
        webhookSecurityService.validate(RepositoryProvider.GITHUB, headers, payload);
        JsonNode root = objectMapper.readTree(payload);
        WebhookPullRequest pullRequest = parsePullRequest(root);
        if (!"pull_request".equals(event) || pullRequest == null) {
        ...
        auditLogService.record("REVIEW_CREATE_WEBHOOK", "REVIEW", review.id(), "GitHub " + review.repositoryName());
    ...
    private WebhookPullRequest parsePullRequest(JsonNode root) {
        ...
        String owner = firstText(root.at("/repository/owner/login"), root.at("/organization/login"));
        String repositoryName = firstText(root.at("/repository/name"), null);
        ...
        String defaultBranch = firstText(root.at("/repository/default_branch"), null);
        if (!StringUtils.hasText(defaultBranch)) {
            defaultBranch = "main";
        }

        String commitSha = firstText(root.at("/pull_request/head/sha"), root.at("/after"));
